...
const DLab = {
  ...
  renderNav(activeHref = '', containerId = 'nav-root') {
    ...
    const studentHtml = s
      ? `<span class="nav-student">${s.id}</span>
  ...
  renderSidebar(activePage = '', containerId = 'sidebar-tree') {
    ...
    const sections = [
      ...
      { id: '3', label: 'Options', color: 'c3', subs: [
      ...
      { id: '4', label: 'Greeks', color: 'c4', subs: [
      ...
      { id: '5', label: 'Simulation & Valuation', color: 'c5', subs: [
    ...
    const html = sections.map(sec => {
      const subItems = sec.subs.map(sub => {
        ...
        const cls = ['tree-item', locked ? 'locked' : '', active ? 'active' : ''].filter(Boolean).join(' ');
        const inner = locked
          ? `<span class="tree-label">${sub.id} ${sub.label}</span><span class="lock-icon">🔒</span>`
          : `<a href="${sub.href}" class="tree-label">${sub.id} ${sub.label}</a>`;
        return `<li class="${cls}">${inner}</li>`;
      }).join('');

      return `
        <div class="tree-section">
          <div class="tree-section-head c${sec.id}-color">
            <span class="tree-section-num">${sec.id}</span> ${sec.label}
          </div>
          <ul class="tree-items">${subItems}</ul>
        </div>`;
    }).join('');
  ...
  initPractice(selector = '.practice-block') {
    document.querySelectorAll(selector).forEach(block => {
      ...
      checkBtn.addEventListener('click', () => {
        ...
        const correct = Math.abs(val - answer) <= Math.abs(answer * tol);
